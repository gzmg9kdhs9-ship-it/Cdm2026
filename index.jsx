...
// ── MATCH DATA — Heures UTC officielles FIFA 2026 ──────────────────────────
const BASE_MATCHES = [
...
function lsSave(key,val) { try{localStorage.setItem("cdm2026_"+key,JSON.stringify(val));}catch{} }
function lsLoad(key) { try{const v=localStorage.getItem("cdm2026_"+key);return v?JSON.parse(v):null;}catch{return null;} }
...
const S = {
  ...
  matchLocked:{background:"#1a0808",border:"1px solid #7f1d1d",opacity:.85},
...
export default function App() {
  ...
  useEffect(() => {
    const p=lsLoad("players"), pr=lsLoad("pronos"), r=lsLoad("results");
  ...
  // Sync toutes les 15s (plusieurs onglets/appareils sur même réseau)
  useEffect(() => {
    const t = setInterval(() => {
      const pr=lsLoad("pronos"), r=lsLoad("results");
      if(pr) setPronos(pr);
      if(r) setResults(r);
      setTick(x=>x+1);
    }, 15000);
  ...
  async function savePlayers(p) { setPlayers(p); setAdminSetup(true); lsSave("players",p); }

  async function savePronosForPlayer(player, data) {
    const merged={...pronos,[player]:{...(pronos[player]||{}),...data}};
    setPronos(merged); lsSave("pronos",merged);
    showToast(`Pronos de ${player} sauvegardés ✓`);
  }

  async function saveResults(data) {
    const merged={...results,...data}; setResults(merged); lsSave("results",merged);
  ...
  async function handleFetchScores() {
    ...
    try {
      const matchList = BASE_MATCHES.filter(m=>m.phase===filterPhase)
        .map(m=>`[ID:${m.id}] ${m.home} vs ${m.away}`).join("\n");
      const resp = await fetch("/api/scores", {
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({phase:filterPhase, matches:matchList}),
      });
      ...
      setResults(merged); lsSave("results",merged);
  ...
  if(loading) return (
    <div style={{...S.app,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,minHeight:"100vh"}}>
      ...
      <div style={{...S.spinner,width:24,height:24,border:"3px solid rgba(255,255,255,.2)",borderTop:"3px solid #16a34a"}}/>
    </div>
  );
    ...
    <div style={S.app}>
      ...
      {screen==="home" && (
        <div style={S.center}>
          <div style={{background:"linear-gradient(135deg,#1e3a5f,#0f172a)",border:"2px solid #16a34a",borderRadius:16,padding:"32px 24px",marginBottom:20,textAlign:"center"}}>
            <div style={{fontSize:46,fontWeight:900,letterSpacing:".04em",marginBottom:6}}>⚽ CDM 2026</div>
          ...
          {!adminSetup ? (
            <div style={S.card}>
              ...
              <p style={{color:"#94a3b8",fontSize:14,marginBottom:16}}>L'admin doit configurer les profils joueurs.</p>
      ...
      {screen==="standings" && (
        <div style={S.page}>
          ...
          {standings.map((s,i)=>(
            <div key={s.name} style={{...S.card,display:"flex",alignItems:"center",gap:12,marginBottom:8,border:i===0?"2px solid #fbbf24":i===standings.length-1?"1px solid #dc2626":"1px solid #334155",opacity:i===standings.length-1?.8:1}}>
...
function AdminSetup({adminMode,adminPass,setAdminPass,showAdminInput,setShowAdminInput,onAuth,players,onSave,showToast}) {
  ...
  return (
    <div style={{maxWidth:500,margin:"0 auto",padding:"20px 16px"}}>
      ...
      <div style={{background:"#1e293b",border:"1px solid #334155",borderRadius:12,padding:20}}>
        <p style={{color:"#94a3b8",fontSize:14,marginBottom:16}}>Créez les profils. Données sauvegardées localement sur chaque appareil.</p>
        {local.map((p,i)=>(
...
function PronoForm({player,pronos,allPronos,players,results,onSave}) {
  ...
  return (
    <div>
      <div style={{...{background:"#1e293b",border:"1px solid #334155",borderRadius:12,padding:16,marginBottom:14},display:"flex",alignItems:"center",gap:14}}>
        ...
        <>
          ...
          {(()=>{
            ...
            return SORTED.map(m=>{
              ...
              return (
                <div key={m.id}>
                  {nd&&<div style={{fontSize:13,fontWeight:700,color:"#93c5fd",background:"#0f1f3d",border:"1px solid #1e3a5f",borderRadius:8,padding:"8px 12px",marginBottom:8,marginTop:16,textTransform:"capitalize"}}>📅 {fmtDay(m.kickoff)}</div>}
          ...
          <button style={{background:"linear-gradient(135deg,#16a34a,#15803d)",color:"#fff",border:"none",padding:"12px",borderRadius:8,fontSize:15,fontWeight:700,cursor:"pointer",width:"100%",marginTop:8}} onClick={()=>onSave(local)}>💾 Sauvegarder mes pronos</button>
        </>
      )}
      ...
      {view==="all" && (
        <div>
          <div style={{background:"#0f1a0f",border:"1px solid #166534",borderRadius:12,padding:14,marginBottom:14}}>
            <p style={{margin:0,fontSize:13,color:"#4ade80"}}>👥 Pronos visibles uniquement après le coup d'envoi.</p>
          </div>
          {(()=>{
            let lastDay=null;
            return SORTED.map(m=>{
    ...
    <div>
      ...
      <div style={{background:"linear-gradient(135deg,#0f1f3d,#0f172a)",border:"2px solid #3b82f6",borderRadius:14,padding:16,marginBottom:20,display:"flex",alignItems:"flex-start",gap:16,flexWrap:"wrap"}}>
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:13,color:"#94a3b8"}}>Phase : <strong style={{color:"#93c5fd"}}>{filterPhase}</strong> — scores via IA + recherche web</div>
          {fetchLog&&<div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:fetchLog.ok?"#052e16":"#1f0a0a",border:`1px solid ${fetchLog.ok?"#166534":"#7f1d1d"}`,fontSize:13,color:fetchLog.ok?"#4ade80":"#f87171"}}>{fetchLog.ok?`✅ ${fetchLog.count} résultat(s) importé(s)`:` ❌ ${fetchLog.error}`}</div>}
        </div>
        <button style={{background:"linear-gradient(135deg,#2563eb,#1d4ed8)",color:"#fff",border:"none",padding:"11px 18px",borderRadius:9,fontSize:14,fontWeight:700,cursor:fetching?"not-allowed":"pointer",opacity:fetching?.6:1,whiteSpace:"nowrap",flexShrink:0,display:"flex",alignItems:"center",gap:8}} onClick={onFetchScores} disabled={fetching}>
          {fetching?<><span style={{width:14,height:14,border:"2px solid rgba(255,255,255,.3)",borderTop:"2px solid #fff",borderRadius:"50%",display:"inline-block",animation:"spin .8s linear infinite"}}/>Recherche...</>:"🔄 Récupérer"}
        </button>
      </div>
      <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:16}}>
        {PHASES.map(ph=><button key={ph} style={{...{background:"#1e293b",border:"1px solid #334155",color:"#64748b",padding:"6px 10px",borderRadius:6,cursor:"pointer",fontSize:12,fontWeight:600},...(filterPhase===ph?{background:"#1e3a5f",border:"1px solid #3b82f6",color:"#93c5fd"}:{})}} onClick={()=>setFilterPhase(ph)}>{ph}</button>)}
      </div>
      {filterPhase==="Groupes"&&(
        <div style={{background:"#1e293b",border:"1px solid #334155",borderRadius:12,padding:16,marginBottom:14}}>
      ...
      {filtered.map(m=>{
        ...
        return (
          <div key={m.id} style={{background:done?"#052e16":"#1e293b",border:`1px solid ${done?"#166534":"#334155"}`,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{flex:1,fontSize:13,fontWeight:600,color:"#e2e8f0",textAlign:"center"}}>{m.home}</span>
              <input style={{background:"#0f172a",border:"1px solid #475569",color:"#f1f5f9",width:48,height:40,textAlign:"center",borderRadius:6,fontSize:18,fontWeight:700,outline:"none"}} type="number" min="0" max="20" placeholder="–" value={local[m.id]?.home??""} onChange={e=>set(m.id,"home",e.target.value)}/>
              <span style={{color:"#475569",fontWeight:700,fontSize:18}}>–</span>
              <input style={{background:"#0f172a",border:"1px solid #475569",color:"#f1f5f9",width:48,height:40,textAlign:"center",borderRadius:6,fontSize:18,fontWeight:700,outline:"none"}} type="number" min="0" max="20" placeholder="–" value={local[m.id]?.away??""} onChange={e=>set(m.id,"away",e.target.value)}/>
              <span style={{flex:1,fontSize:13,fontWeight:600,color:"#e2e8f0",textAlign:"center"}}>{m.away}</span>
            </div>
          </div>
        );
      })}
      <button style={{background:"linear-gradient(135deg,#16a34a,#15803d)",color:"#fff",border:"none",padding:"12px",borderRadius:8,fontSize:15,fontWeight:700,cursor:"pointer",width:"100%",marginTop:8}} onClick={()=>onSave(local)}>💾 Sauvegarder</button>
    </div>
  );
}
  ...
  return (
    <div style={{marginTop:32}}>
      ...
      {filtered.length===0?<p style={{color:"#4b5563",textAlign:"center"}}>Aucun résultat.</p>:filtered.map(m=>{
        ...
        return (
          <div key={m.id} style={{background:"#1e293b",border:"1px solid #334155",borderRadius:12,padding:16,marginBottom:8}}>
